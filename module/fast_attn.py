...
class FastMultiHeadedAttention(nn.Module):
    ...
    def rel_attn(q, k, v, start_nodes, end_nodes, rel_q, rel_k, rel_v):
        ...
        :param start_nodes: [batch_size, num_heads, k+1, seq_len] 需要将其映射为偏移位置, 同时只保留上半矩阵
        ...
        # 还原到seq_len * seq_len的矩阵计算attn_weights
        # 移除偏移位置
        ...
        # 对最后一维度重排序，变成1,2,3...n的顺序
        score_sum = torch.zeros(scores.size(), device=scores.device)
        score_sum.scatter_add_(dim=-1, index=query_indexes, src=scores)  ## 这里是不是可以优化
        # 防止出现一行全为0的情况
        ...
        # 和V做点积  shape [batch_size, num_heads, 2(k+1), seq_len, d_k]
